...
export class KeyboardLayoutsComponent implements OnInit, AfterViewInit {
  ...
  fontsSources: string[] = ['ari','chrs','dogr','elym','gong','gonm','hmnp','khar','khud','kult','luw','maka','moss','nand','nshu','odu','ougr','sgnw','sog','soyo','txg','ur','vatt','yezi','zag','zanb'];
  fontClass: string = "";

  // Words Suggestion for All Supported Languages
  supportedLanguages : string[] = ['af','am','ar','ari','az','bak','be','befr','bg','bn','bopo','br','brah','bs','bsk','ca','ceb','co','cs','cy','da','de','el','en','engb','enin','enintl','enus','eo','es','esmx','et','eu','fa','fi','fj','fo','fr','frca','fy','ga','gd','gl','gn','goth','gu','gv','ha','haw','he','hi','hmn','hr','ht','hu','hy','id','ig','ilo','is','it','ja','jv','ka','kk','km','kn','ko','gom','kon','ku','kw','ky','la','lb','lfn','ln','lo','lt','lv','mg','mi','mk','ml','mn','mr','ms','mt','my','nag','ne','nl','nld','no','ny','nya','odu','oji','or','pa','pin','pl','ps','pt','ptbr','qu','rn','runr','ro','rom','ru','rw','sa','sank','sd','si','sk','sl','sm','sn','so','sq','sr','st','su','sv','sw','ta','te','tfng','tg','th','tk','tl','tpi','tr','tt','ty','ug','uk','ur','uz','vi','xh','yi','yo','zhcn','zhtw','zu'];
  ...
  async populateSuggestionsForLanguage(ISO_Code) {
    ...
    if (ISO_Code == this.sessionManager.getFromSessionURL() && this.sessionManager.getOfflineOnly() == false)
      ...
      var self = this;
      var language_script_name = (this.keyboardLayouts[ISO_Code][2].indexOf("(") > -1 && ISO_Code != "zhcn" && ISO_Code != "zhtw") ? this.keyboardLayouts[ISO_Code][2].toLowerCase().split("(")[0] : ((ISO_Code == "runr") ? "runic": this.keyboardLayouts[ISO_Code][2].toLowerCase());
  ...
  alphabetType(tabChangeEvent: MatTabChangeEvent) {
    ...
    } else if (tabChangeEvent.index == 56 && tabChangeEvent.tab.textLabel == "Sunuwar") {
  ...
  abugidaType(tabChangeEvent: MatTabChangeEvent) {
    ...
    } else if (tabChangeEvent.index == 79 && tabChangeEvent.tab.textLabel == "Ngahi Mbojo") {
      this.sessionManager.setInSessionURL('bhp');
    ...
    } else if (tabChangeEvent.index == 83 && tabChangeEvent.tab.textLabel == "ಬ್ಯಾರಿ-Byari") {
      this.sessionManager.setInSessionURL('bya');
    }
    this.layoutCurrentKeys = this[this.keyboardLayouts[this.sessionManager.getFromSessionURL()][3]];
    this.resetForAKeyboard();
    this.initSwara();
  ...
  keyPressed(element, value, action, type, src) {
    ...
    if (action === "shift") {
      ...
      this.resetSwara();
      if (this.unicode5AndHigher && this.sessionManager.getFromSessionURL() != "runr")
        this.sessionManager.setCharFromKeyboard("　");
      else
  ...
  typeVertically(){
    this.sessionManager.typeVertically.next(!this.sessionManager.typeVertically.value);
  }
  ...
  verifyThisKeyboardHasQwerty() {
    ...
    if (qwertyExists == false && this.layoutCurrentKeys && this.layoutCurrentKeys.length > 0)
